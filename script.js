...
$(document).ready(() => {
 ...
 // إنشاء عنصر <textarea> مؤقت لنسخ النص منه
 var tempTextarea = document.createElement('textarea');
 tempTextarea.value = element.textContent;
 document.body.appendChild(tempTextarea);
 // تحديد النص داخل العنصر <textarea>
 tempTextarea.select();
 tempTextarea.setSelectionRange(0, 99999); // لأجهزة الموبايل
 // نسخ النص إلى الحافظة
 document.execCommand('copy');
 // إزالة العنصر <textarea> المؤقت
 document.body.removeChild(tempTextarea);
 // تنبيه المستخدم بأن النص تم نسخه
 }
 ...
 newDiv.style=`
 position:fixed;
 margin-block:-30%; margin-left:0%;background-color:gray;width:95%;height:70%;
 border:double;
 z-index:1000; 
 `
 ...
 btnCopy.append(document.createTextNode("COPPY"))
  ...
  btnCopy.setAttribute("onclick",`copyToClipboard(content)`)
  newDiv.append(text)
 ...
 div.append(newDiv) 
  ...
  server.setAttribute("onclick",`copyServer()`);